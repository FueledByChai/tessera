...
- A Studies page: pick a grid (lake seconds, or daily / 5-minute / 1-minute CSV bars with typed symbols), instruments, features (including exogenous series from `[[data.series]]` in `local.toml`, and funding / open interest on the lake, joined as-of availability), horizons, target, mode (time-series or cross-sectional), and decision delay; `agg daily` features on the daily grid lift from the intraday library, and every registered event series gets an event-path table; the selected cell shows its diagnostics (incremental IC against the accepted set, the per-day IC strip, regime table, autocorrelation); the API runs the study and the page shows the IC heat table (feature by horizon, with top-minus-bottom decile bps, breakeven cost, and t-stat on hover; rows sort by breakeven cost, |IC|, or name), a decile table, the costless equity curve of the selected cell (clipped z-score or sign position; Sharpe, turnover, breakeven), and a ranked table of every cell by breakeven cost.
