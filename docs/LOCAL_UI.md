...
- One run form for every SDK strategy: universe or explicit symbols (no cap on the number of symbols), bar resolution, session, position size, minimum entry price, maximum gross exposure (buying power), capital, entry limits, and the manifest parameters grouped by tier, with explicit dates and research labels.
...
- A Studies page: pick a grid (lake seconds, or daily / 5-minute / 1-minute CSV bars with typed symbols), instruments, features (including exogenous series from `[[data.series]]` in `local.toml`, and funding / open interest on the lake, joined as-of availability), horizons, target, mode (time-series or cross-sectional), and decision delay; `agg daily` features on the daily grid lift from the intraday library, and every registered event series gets an event-path table; the selected cell shows its diagnostics (incremental IC against the accepted set, the per-day IC strip, regime table, autocorrelation); a feature library panel keeps named expressions in the catalog database (`GET`/`POST /api/features`, `POST /api/features/{id}/promote`, `DELETE /api/features/{id}`): "use" drops one into the candidates, "promote" puts it in the accepted set that every study regresses candidates against (a promoted feature that cannot run on the chosen grid is left out of that study), and the selected cell in the results can be promoted in place; a study with an accepted set also writes `accepted.parquet` (symbol, bar close, date, one column per accepted feature, one target column per horizon), downloadable from the results strip (`GET /api/studies/{id}/export`) and readable with `tessera parquet-schema`; the API runs the study and the page shows the IC heat table (feature by horizon, with top-minus-bottom decile bps, breakeven cost, and t-stat on hover; rows sort by breakeven cost, |IC|, or name), a decile table, the costless equity curve of the selected cell (clipped z-score or sign position; Sharpe, turnover, breakeven), and a ranked table of every cell by breakeven cost.
...
- A Data sources inventory on the Data page: every configured source (CSV bar library feeds, catalog and universe lists, freshness, update command; the parquet tick lake's feeds and instruments) with paths, file counts, sizes, and first/last dates, a Rescan button, and a Configure sources panel with the `local.toml` template and the steps for adding a source. See [DATA_SOURCES.md](DATA_SOURCES.md).
