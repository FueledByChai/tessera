...
- One run form for every SDK strategy: universe or explicit symbols (no cap on the number of symbols), bar resolution, session, position size, minimum entry price, capital, entry limits, and the manifest parameters grouped by tier, with explicit dates and research labels.
