...
1. Run `scripts/backlog-status.sh --next`. It names the first `todo` ticket in `BACKLOG.md`
   ...
   `doing` in the file, and claim it once the worktree exists:
...
5. Commit with the ticket id first in the subject, a body that says what changed and how the
   ...
   claim in `BACKLOG.md` (the heading keeps no state; git records done). Then
   `scripts/open-ticket-pr.sh <id>` pushes the branch and opens the pull request against `main`
   (`--body-file` for a fuller report than the commit body). Never push `main`.
