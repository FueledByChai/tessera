...
WEB_RUNTIME="$HOME/Library/Caches/Backtester/web-runtime"

mkdir -p "$LOG_DIR"

if ! curl -fsS "$API_URL" >/dev/null 2>&1; then
  if [ ! -x "$ROOT/target/release/backtester_ui" ]; then
    osascript -e 'display notification "Building the local service for the first launch" with title "Backtester"'
    (cd "$ROOT" && cargo build --release --bin backtester_ui --bin backtester) >"$LOG_DIR/build.log" 2>&1
  fi
  (cd "$ROOT" && BACKTESTER_ROOT="$ROOT" nohup "$ROOT/target/release/backtester_ui" >"$LOG_DIR/api.log" 2>&1 & echo $! >"$STATE_DIR/api.pid")
fi
...
until curl -fsS "$API_URL" >/dev/null 2>&1 && curl -fsS "$WEB_URL" >/dev/null 2>&1; do
  ...
  if [ "$attempt" -ge 60 ]; then
    osascript -e 'display alert "Backtester did not start" message "Check data/ui/logs/api.log and web.log." as critical'
    exit 1
  fi
  sleep 1
done
