...
read_config() {
  ...
  perl -e '
    ...
    my @order = qw(default_branch backlog check check_fast review_paths trailer_required);
...
EOF
  ...
  got="$(LOOP_CONFIG="$dir/other.toml" "$me" check_fast)"; [ "$got" = "npm run lint" ] || { echo "self-test: check_fast should be set, got '$got'"; exit 1; }
  got="$(LOOP_CONFIG="$dir/other.toml" "$me" review_paths)"; [ "$got" = 'schema/"quoted".json' ] || { echo "self-test: escaped quote lost: '$got'"; exit 1; }
  got="$(LOOP_CONFIG="$dir/other.toml" "$me" --all | grep -c '=')"; [ "$got" = 6 ] || { echo "self-test: --all should print six keys, got $got"; exit 1; }
