...
#                                     chart, run form, costs, and data page checks skip themselves
#                                     without a Chromium
...
step "loop self-tests: config, backlog status, ticket PRs, release notes, kit sync, proof gate, coverage ratchet, review status, decisions, prompt check, sprint, deploy, scratch console"
...
scripts/scratch-console.sh --self-test
...
if [ "$NO_WEB" = 0 ]; then
  step "web typecheck, lint, theme check, build, layout, chart, run form, costs, and data page checks"
  (cd web && npm run --silent typecheck && npm run --silent lint && npm run --silent theme-check && npm run --silent build >/dev/null && npm run --silent layout-check && npm run --silent chart-check && npm run --silent run-form-check && npm run --silent costs-check && npm run --silent data-page-check)
fi
